<script lang="ts">
	...
	let downloadStatus: DownloadCallback = $state(new DownloadCallback("Inactive", DownloadState.Inactive, 0))
	...
	onMount(async () => {
		...
		ws.addEventListener("open", (e)=> {
			const initDTO = new DownloadDTO(DownloadDTOType.init, {
				downloadURL: "https://github.com/Delfi-CH/remind-me/releases/download/latest/remind_me-android.apk",
				downloadPath: "/tmp",
...
<p>Download status: {DownloadState[downloadStatus.state]} message: {downloadStatus.message} progress: {Math.floor(downloadStatus.progress ?? 0)}% total: {downloadStatus.total} speed: {downloadStatus.speed}</p>
<button onclick={()=>{
